...
function child_enqueue_styles()
{
    ...
    wp_register_style('my-style', get_stylesheet_directory_uri() . '/css/custom_checkout.css');
    wp_enqueue_style('my-style');
    wp_register_script('hello-elementor-child-theme-jquery', 'https://cdnjs.cloudflare.com/ajax/libs/jquery/3.7.1/jquery.min.js');
    wp_enqueue_script('hello-elementor-child-theme-jquery');
    wp_register_script('hello-elementor-child-theme-js', get_stylesheet_directory_uri() . '/js/custom_checkout.js');
    wp_enqueue_script('hello-elementor-child-theme-js');
}

add_action('wp_enqueue_scripts', 'child_enqueue_styles', 15);
// Your custom plugin code

function remove_woocommerce_coupon_form()
{
    remove_action('woocommerce_before_checkout_form', 'woocommerce_checkout_coupon_form', 10);
    remove_action('woocommerce_checkout_order_review', 'woocommerce_checkout_payment', 20);
}

// Hook this function to a WordPress action hook
add_action('init', 'remove_woocommerce_coupon_form');
add_action('custom_run_woocommerce_checkout_coupon_form', 'woocommerce_checkout_coupon_form');
add_action('custom_woocommerce_checkout_payment', 'woocommerce_checkout_payment');


// Define a custom shortcode function
function checkout_steps_fun()
{
?>
    <div class="main_pro_cont">
        <div class="inner_h_cont">
            <div class="h_steps billing_step ad_slash active">
                <div class="h_num">1</div>
                <span class="h_tite">Versand</span>
            </div>
            <div class="h_steps payment_step ad_slash">
                <div class="h_num">2</div>
                <span class="h_tite">Zahlung</span>
            </div>
            <div class="h_steps final_step">
                <div class="h_num">3</div>
                <span class="h_tite">Fertig</span>
            </div>
        </div>
    </div>
<?php
...
add_shortcode('checkout_steps', 'checkout_steps_fun');
